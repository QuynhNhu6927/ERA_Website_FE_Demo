...
export function CompassMergeAnimation({ cycle }: CompassMergeAnimationProps) {
  ...
  return (
    <div className="relative w-96 h-52 md:w-[36rem] md:h-52">
      ...
      {/* Flash effect — behind circles, perfect circle */}
      <AnimatePresence>
        {step >= 5 && step < 6 && (
          <motion.div
            className="absolute left-1/2 top-1/2 z-[-1] w-36 h-36 md:w-52 md:h-52 rounded-full bg-[#1E40AF] pointer-events-none"
            style={{
              x: "-50%",
              y: "-50%",
              filter: "blur(32px)",
      ...
      {step >= 6 && (
        <motion.div
          ...
          animate={{
            scale: step >= 11 ? 0 : bigCircleScale,
            opacity: step >= 11 ? 0 : 1,
          }}
          transition={{
            scale: { type: "spring", stiffness: 260, damping: 18 },
        ...
        >
          {/* Neon glowing orbs behind circle — bigger and deeper inset */}
          {step >= 9 && step < 10 && (
            <>
              <motion.div
                className="absolute inset-[-20px] md:inset-[-32px] rounded-full pointer-events-none"
                animate={{ rotate: 360 }}
                transition={{ duration: 5, ease: "linear" }}
              >
                <div
                  className="absolute top-[30%] left-1/2 -translate-x-1/2 -translate-y-1/2 w-32 h-32 md:w-44 md:h-44 rounded-full"
                  style={{
